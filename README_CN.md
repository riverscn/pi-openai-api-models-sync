...
1. 从 `~/.pi/agent/models.json` 读取已有 provider。
2. 使用该 provider 配置的认证信息请求 `<baseUrl>/models`。
3. 获取模型价格与能力元数据。
...
创建 `~/.pi/agent/pi-openai-api-models-sync.json`：
...
对应 provider 必须已经存在于 `~/.pi/agent/models.json`：
...
| 字段 | 默认值 | 说明 |
| --- | --- | --- |
| `providerId` | 必填 | `models.json` 中的 provider key |
| `pricingUrl` | Sub2API 元数据仓库 | 价格和模型能力 JSON 地址 |
| `include` | `[".*"]` | 正则列表，模型至少匹配一项才会保留 |
| `exclude` | audio/realtime/image/embedding | 匹配这些正则的模型会被排除 |
